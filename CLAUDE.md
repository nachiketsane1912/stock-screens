...
- **`pages/screens.py`** — runs screens across the whole universe of companies (SSGR, Moats, Nalanda's F, CCP tabs) and lists who passes each.
...
- **Slow (fundamentals)** — unchanged: `EPS_TTM` and `Shares_Latest` now ride along as two extra columns in `build_universe_cache()`'s per-company loop (`income_statement.loc["EPS", "TTM"]` and `balance_sheet.loc["NOS", balance_sheet.columns[0]]` — the latest FY-end share count, since BS has no TTM column and its columns are most-recent-first). They're cheap byproducts of pivots the loop already builds, and only need refreshing as often as fundamentals do.
- **Fast (Price/P·E/Market Cap)** — an optional `Market` sheet in the workbook (`Symbol`, `CMP`, `PE`, `Market cap (INR Cr)`, `Beta`), re-pasted from a data vendor as often as the user likes. **These three figures are read directly from the sheet, not derived** — deliberately: the vendor already knows real current shares outstanding and reported EPS, which this app can't reproduce as accurately from the annual workbook alone (its own Market Cap/P·E would need `Shares_Latest`/`EPS_TTM` approximated from the latest completed fiscal year, which lags actual current values). `merge_market_data(universe, sheets)` (`data_loader.py`) left-merges `Market`'s `CMP`/`PE`/`Market cap (INR Cr)` onto the universe DataFrame by `Symbol`, renaming to `Price`/`PE`/`Market Cap (Cr)`. This function is **deliberately uncached** — a plain merge over ~2000 rows is fast enough to redo on every rerun, so `pages/screens.py` calls it right after resolving `universe` (cached or freshly built) and every tab downstream sees Price as of the workbook's last save, with no Refresh click needed for it. If `sheets` has no `"Market"` key, `merge_market_data` returns `Price`/`Market Cap (Cr)`/`PE` all `NaN` rather than raising, so the app works unchanged for a workbook that hasn't added the sheet yet.
...
`pages/data_explorer.py` shows a **Filters** table (right after the company header, before the detailed financial tables) with a row for every screen across the whole app — SSGR + the 7 Moats + the 4 Nalanda's F filters + the 2 CCP lists (14 rows) — giving that one company's verdict on each, and *why* it failed when it did.
